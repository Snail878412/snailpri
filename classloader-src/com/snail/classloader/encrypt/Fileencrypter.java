package com.snail.classloader.encrypt;
...
public class Fileencrypter {
	...
	private static Logger log = Logger.getLogger(Fileencrypter.class);
	...
	public static void main(String args[]) {
		...
		String pass3 = pass.substring(4);
		 String fileName =
		 "E:\\个人资料\\workspace\\ClassLoader\\bin\\com\\snail\\classloader\\test\\Animal.class";
//		String fileName = "D:\\test.txt";

		 da.encrypt(new File(fileName),
		 da.md5s(pass1) + da.md5s(pass2) + da.md5s(pass3));
//		da.decrypt(new File(fileName),
//				da.md5s(pass1) + da.md5s(pass2) + da.md5s(pass3));
	}
	...
	public byte[] encrypt(byte[] bytIn, String sKey) throws SPException {
		if (StringUtils.isBlank(sKey) || sKey.length() != 48) {
			throw new SPException("请设置加密盐");
		}
		byte[] bytK1 = getKeyByStr(sKey.substring(0, 16));
		byte[] bytK2 = getKeyByStr(sKey.substring(16, 32));
		byte[] bytK3 = getKeyByStr(sKey.substring(32, 48));
		...
		try {
			...
			throw new SPException("加密失败");
		}
	}

	public byte[] decrypt(byte[] bytIn, String sKey) throws SPException {
		if (StringUtils.isBlank(sKey) || sKey.length() != 48) {
			throw new SPException("请设置解密盐");
		}
		try {
			byte[] bytK1 = getKeyByStr(sKey.substring(0, 16));
			byte[] bytK2 = getKeyByStr(sKey.substring(16, 32));
