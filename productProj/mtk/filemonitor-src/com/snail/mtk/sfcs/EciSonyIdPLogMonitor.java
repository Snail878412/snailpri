...
import com.snail.mtk.sfcs.monitor.handler.EciSonyIdpLogHandler;
...
public class EciSonyIdPLogMonitor {
	...
	// 防止启动多个.
		...
		public static void main(String[] args) {
			...
			EciSonyIdpLogService service = null;
			try {
				service = rmiClient.getRmiService("eciSonyIdpLogService", EciSonyIdpLogService.class);
			} catch (Exception e) {
				e.printStackTrace();
			}
			if (service == null) {
				System.out
						.println("监视器启动失败:没有找EciSonyIdpLogService,或不是EciSonyIdpLogService类.");
				return;
			}
			...
			while (it.hasNext()) {
				...
				EciSonyIdpLogHandler fileHandler = (EciSonyIdpLogHandler) fileHandlerObj;
			...
			EciSonyIdPLogMonitor.running = true;
		}
